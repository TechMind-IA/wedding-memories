...
export default function AdminLoginPage() {
  ...
  const [coupleNames, setCoupleNames] = useState("Brenda & Jonathas")
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([authData, infoData]) => {
        if (authData.authenticated) {
          router.replace("/admin/dashboard")
        } else {
          setIsChecking(false)
        }
        if (infoData.coupleNames) {
          setCoupleNames(infoData.coupleNames)
        }
      })
  ...
  if (isChecking) {
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-background">
        <p className="text-muted-foreground font-sans">Verificando...</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-background px-4">
      {/* Botão voltar — topo da página */}
      <Link
        href="/"
        className="fixed top-5 left-5 z-20 inline-flex items-center gap-2 font-sans text-sm text-muted-foreground hover:text-foreground transition-colors"
      >
